...
**约束**：默认摘要阈值为 512000 token、压缩目标为 320000 token、最近完整工具结果预算为 192000 token；token 估算必须包含正文、reasoning 和工具参数并使用安全系数；快照更新失败不得丢失已经落库的 `agent_run`，下一轮必须按 `last_applied_run_id` 自动补偿；清空会话历史时必须同时删除聊天消息、运行账本和上下文快照。
