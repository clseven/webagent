...
> 修订：2026-07-13 将“双次自检、第三次放行”改为“保存首版候选、单次验证、通过后原样放行”。
>
...
2. 用候选答案单次验证 + 现有 `MAX_ITERATIONS`，替代预算闸。
...
- **触发**：模型准备给最终答案时。
- **判定方式（B 方案）**：harness **不解析**模型输出。模型继续调工具 → 候选失效并循环补查；模型再次收尾 → 自检通过并原样放行候选。
- **候选保护**：自检轮生成的正文只用于表达“再次收尾”行为，不得覆盖自检前候选答案。
...
模型准备收尾
  ...
  │
  └─ TodoState 干净 → 保存首版候选并注入一次证据自检：
       模型继续调工具 → 旧候选失效，补查后生成并验证新候选
       模型再次收尾   → 自检通过，原样放行首版候选
```

两个关键细节：

- **TodoState 降级为前置信号**，不再是充分条件。没闭环一定拦；但闭环了不代表证据够，仍要走自检。
- **工具调用先使候选失效**。即使工具随后被 Pre Hook 拦截，也不能复用已被模型判定有问题的旧答案。
- **每个候选只验证一次**。没有问题时不重复生成最终正文；持续补查仍由 `MAX_ITERATIONS` 防止无限循环。
...
- **候选答案快照** → 自检通过只确认，不重新生成最终交付正文。
- **靠行为不靠解析** → 模型二选一行动（继续调工具 / 再次收尾），harness 不解析任何标记。
...
| 预算闸（搜索次数硬拦） | 候选单次验证 + `MAX_ITERATIONS`：harness 控制何时必须停 |
...
- 新增证据自检分支：TodoState 干净时返回 `VERIFY_CANDIDATE`，由执行器保存候选并注入 3.3 的提示。
- Todo 未闭环时返回普通 `CONTINUE`，不得误保存尚未满足硬条件的候选答案。
...
### 4.5 改造：`ReactAgent.java`（候选答案生命周期）

路径：`src/main/java/com/example/sandbox/web/service/impl/ReactAgent.java`

这是方案里唯一需要动 `ReactAgent` 主循环的地方，需谨慎：

- `StopHook` 返回结构化 `StopDecision`：`ALLOW`、`CONTINUE`、`VERIFY_CANDIDATE`。
- `VERIFY_CANDIDATE` 保存当前正文和 reasoning；下一轮再次收尾时返回快照，不采用自检轮正文。
- 自检轮出现工具调用时，在执行 Pre Hook 之前使候选失效，防止工具被拦后错误复用旧答案。
- 同步路径（`run`）和流式路径（`runStream`）必须保持一致；流式最终 `answer` 事件也使用通过验证的候选。
...
1. **先做 4.5 的状态机制**：在 `ReactAgent` 加候选快照和结构化 Stop 决策。
2. **再做 4.1 + 4.2**：升级 `FinalTodoGuardHook`，区分普通继续与候选验证。
3. **验证**：覆盖无问题原样放行、自检补查后旧候选失效、同步与流式结果一致。
...
| 风险 | 兜底 |
|---|---|
| 模型过度自信，反复输出答案 | 第一次再次收尾即视为通过，原样放行候选，不重复自检 |
| 模型过度谨慎，一直判"不够"无限查 | 现有 `MAX_ITERATIONS` 兜底 |
| 模型敷衍走自检流程 | 主模型为 DeepSeek V4，推理能力足够；上线后观察自检质量 |
| 自检轮改写答案 | 最终 `answer` 和持久化正文固定使用自检前候选快照 |
| 自检被写进 content 而非 reasoning | 自检轮正文不进入最终答案；提示仍要求检查过程只写入 reasoning |
| 删除 `AgentSearchPolicyHook` 后搜索失控 | 自检第 1 条覆盖方向判断；候选验证 + MAX_ITERATIONS 覆盖收敛；上线后对比 token 消耗 |

## 七、未纳入本方案（后续）

- **EvidenceStore（结构化证据快照）**：当前自检让模型自己翻对话历史，1M 上下文扛得住。等自检质量不够再上 Post Hook 维护结构化证据快照，喂给自检提示，提升判断准确度。
- **state check 那道自检**：已独立成计划，见第八章。
- **预算运行时模块**：通用预算器（按会话/任务给所有工具配额）。当前由候选验证 + MAX_ITERATIONS 承担收敛，后续若需要更细的工具预算控制再做。
...
- **测试用例**：①首版候选进入一次自检；②无问题时原样放行首版候选；③自检调用工具后旧候选失效并验证新候选；④TodoState 未闭环时普通拦截且不保存候选；⑤同步与流式最终正文一致。
...
5. B 方案 + 候选答案快照 + 该查才查自检提示（2026-07-13 修订定稿）
