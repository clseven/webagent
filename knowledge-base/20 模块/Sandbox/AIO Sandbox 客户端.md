...
| 文件读取 | `client.files().readText(path)` | 读取整文件文本 |
| 文件读取（分页） | `client.files().readText(path, startLine, endLine)` | 按 0 基行范围读取（end 不含），服务端分页 |
...
各 `Aio*Api` 的请求体不再用 `Map.of` 拼装，而是封装成各域 `model` 子包下的 record，字段与可选性直接由 record 定义，无需查 OpenAPI：

| 域 | 请求 record |
| --- | --- |
| file | `FileReadRequest`、`FileWriteRequest`、`FileReplaceRequest`、`FileSearchRequest`、`FileListRequest` |
| shell | `ShellExecRequest`（POJO）、`ShellViewRequest`、`ShellWaitRequest`、`ShellKillProcessRequest` |
| node | `NodeExecuteRequest` |
| utility | `ConvertToMarkdownRequest` |
| browser | `BrowserConfigRequest` |

record 统一 `@JsonInclude(NON_NULL)` + `@JsonProperty` 蛇形映射，可选字段为 null 时省略。响应侧用 `AioHttpClient.postData(path, req, Class)` 把 `data` 反序列化成类型对象（如 `NodeExecuteResult`、`AioSandboxContext`）。

保持 `Map` 透传的两处：`AioFileApi.edit(Map)`（Anthropic str_replace_editor 协议）和 `AioBrowserApi.action(Map)`（body 字段随 `action_type` 动态变化）。
