...
- 检索增强是六步流程：Query Rewrite → 并行向量召回 → 融合去重 → Rerank → 截取 topK → 构建上下文。
- 知识增强失败不中断主对话，降级为空上下文，对回答质量有影响但不让 Agent 主流程失败。
- Agent 应用关联知识库后，`AgentKnowledgeContextService` 负责加载应用配置、构建工具描述和生成增强上下文。
...
| `rag.enhancement.retrieve.min-score` | `0.5` | 最低召回分数 |
| `rag.enhancement.rerank.top-k` | `8` | Rerank 后保留数量 |

Query Rewrite 当前默认走 DeepSeek 配置回退，不是旧文档里的 Ollama/qwen3。
...
入口：`KnowledgeEnhancerImpl.enhance` 或前端手动检索
...
6. 构建上下文：补充文档名和切片正文，格式化为 `## 知识库检索结果` 文本。
...
- 并行检索用 `CompletableFuture.supplyAsync`，单个 query 或单个知识库检索失败只记日志，不影响其他结果。
...
当 `TurnPolicy` 为 SOCIAL 时，`shouldInjectKB()` 为 false，知识库增强整条跳过，`knowledge_search` 工具也不注入。只有 TASK 和 AMBIGUOUS 轮次才会执行知识库增强。
...
| 检索结果质量差 | 切片参数、Query Rewrite、retrieve topN、rerank topK、minScore | minScore 过高会过滤掉有效候选；topN 过低会漏召回 |
...
| 检索增强未触发 | 应用是否关联知识库、`rag.enhancement.enabled`、TurnPolicy 是否 SOCIAL | SOCIAL 轮次整条跳过 |
...
4. Query Rewrite 当前走 DeepSeek 回退，如果需要独立配置，可以参考 `RagConfigProperties` 增加独立的 rewrite 模型配置。
5. 检索增强的并行检索用 `CompletableFuture.supplyAsync`，默认走 ForkJoinPool，高并发下可能争抢线程；如果出现检索延迟，可以指定自定义线程池。
...
`AgentKnowledgeContextService` 是 Agent 知识库上下文服务，负责加载应用配置、构造知识库工具描述和生成增强上下文。当应用未关联知识库或 TurnPolicy 为 SOCIAL 时，相关方法返回空或跳过。
...
`RerankService` 对向量召回结果二次排序，让最终返回的切片更贴近原始问题。当前使用 BGE Rerank，保留数量由 `rag.enhancement.rerank.top-k` 控制。
